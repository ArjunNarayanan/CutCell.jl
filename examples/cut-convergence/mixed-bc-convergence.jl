...
function solve_and_compute_displacement_error(
    ...
    CutCell.assemble_traction_force_component_linear_form!(
        sysrhs,
        x -> exact_stress(analyticalsolution, x)[1],
        basis,
        facequads,
        cutmesh,
        x -> onrightboundary(x, width, width),
    ...
    CutCell.assemble_traction_force_component_linear_form!(
        sysrhs,
        x -> exact_stress(analyticalsolution, x)[2],
        basis,
        facequads,
        cutmesh,
        x -> ontopboundary(x, width, width),
...
df = DataFrame([dx,u1err,u2err],["Element size", "U1 Error", "U2 Error"])
filename = "examples/cut-convergence/mixed-bc-error.csv"
