...
componenterr = [[se[i] for se in stresserr] for i = 1:4]
rates = [convergence_rate(dx,ce) for ce in componenterr]

df = DataFrame("Element Size" => dx,
               "S11 Error" => componenterr[1],
               "S11 Rate"  => rates[1],
               "S22 Error" => componenterr[2],
               "S22 Rate"  => rates[2],
               "S12 Error" => componenterr[3],
               "S12 Rate"  => rates[3],
               "S33 Error" => componenterr[4],
               "S33 Rate"  => rates[4])
foldername = "examples/transformation-strain/"
filename = "order"*string(polyorder)*"-stress-convergence.csv"
