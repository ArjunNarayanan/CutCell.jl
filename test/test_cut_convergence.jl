...
function solve_plane_interface(x0, normal, nelmts, polyorder, numqp, theta)
    ...
    err =
        mesh_L2_error(disp, x -> nonpoly_displacement(alpha, x), basis, cellquads, cutmesh)
    return err
end

function solve_curved_interface(xc, radius, nelmts, polyorder, numqp, penaltyfactor)
    L = 1.0
    W = 1.0
    lambda, mu = 1.0, 2.0
    dx = 1.0 / nelmts
